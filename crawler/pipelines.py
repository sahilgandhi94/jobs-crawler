    ...
    def process_item(self, item, spider):
        if spider.name in ['sector', 'sector1']:
            if isinstance(item, SectorItem):
                ...
                KEYWORDS = ['hr consultant', 'hr services', 'placement', 'human resource', 'staff', 'manpower', 'sourcing', 'employment', 'rozgar']

                company_name = item['company_name']
                if company_name is not None:
                    company_name = company_name.strip()
                    for name in NAMES+KEYWORDS:
                        if __contains(name, company_name):
                            raise DropItem("Dropped item based on filtering {}".format(company_name))
                number = item['number']
                if number is not None and len(number) > 0:
                    number = number.strip()
                    for phone in NUMBERS:
                        if __contains(phone, number):
                            raise DropItem()
        return item
