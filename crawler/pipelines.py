    ...
    def process_item(self, item, spider):
        if isinstance(item, SectorItem):
            ...
            try:
                try:
                    phone = item['number']
                    if phone is None or len(str(phone)) < 8:
                        return item
                    if item['company_name'] is None or len(item['company_name'].strip()) == 0:
                        company_name = 'NA'
                    else:
                        company_name = item['company_name']
                    table = dynamodb.Table('phone_no_leads')
                    table.put_item(Item={'phone': phone, 'company': company_name,
                                         'source': str(spider.name), 'timestamp': int(time.time())})
                except KeyError:
                    pass
    ...
    def process_item(self, item, spider):
        ...
        if spider.name in ['sector', 'sector1']:
            if isinstance(item, SectorItem):
                ...
                KEYWORDS = ['hr consultant', 'hr services', 'placemen', 'human resource', 'staff', 'manpower', 'sourcing', 'employment', 'rozgar', 'rozgaar', 'hiring for', 'consultan', 'consultancy', 'work from home', 'part time', 'online']
                ...
                try:
                    ...
                    if company_name is not None and len(company_name) > 0:
                        ...
                        for keyword in NAMES+KEYWORDS:
                            if __contains(company_name, keyword):
                                drop = True
                    ...
                    print("Dropped item based on filtering {}".format(company_name))
                    raise DropItem("Dropped item based on filtering {}".format(company_name))
                except Exception as exc:
                    print("Suppressing Exception {}".format(traceback.format_exc()))
                    pass
        if not drop:
            return item
